...
class Scenario:
    ...
    psi_s_weight_scale: float = 1.0
    async_loss: bool = False
    resource_stress: bool = False
    routing_delay_stress: bool = False
    resource_budget_scale: float = 0.55
    resource_demand_multiplier: float = 1.25
    resource_high_demand_bonus: float = 0.20
    resource_replenish_rate: float = 0.0012
    routing_delay_fraction: float = 0.35
...
def scenarios() -> List[Scenario]:
    return [
        ...
        Scenario(
            "hub_2_unbalanced_secondary_failure",
            "hub_redundancy",
            "hub_redundant",
            0.026,
            0.078,
        ...
        Scenario(
            "topology_lag_governance_capture",
            "topology_impact",
            "ring_mesh",
            0.027,
            0.075,
        ...
        Scenario(
            ...
            resource_budget_scale=0.62,
            resource_demand_multiplier=1.18,
            resource_high_demand_bonus=0.12,
            resource_replenish_rate=0.0016,
            routing_delay_fraction=0.28,
        ),
        Scenario(
            "topology_lag_identity_shock",
            "topology_impact",
            "ring_mesh",
            0.027,
            0.075,
        ...
        Scenario(
            ...
            0.017,
            resource_stress=True,
            resource_budget_scale=0.95,
            resource_demand_multiplier=1.04,
            resource_high_demand_bonus=0.02,
            resource_replenish_rate=0.0028,
        ),
        Scenario(
            "resource_pressure_mid",
            "recovery_resource",
            "ring_mesh",
            0.026,
            0.070,
            ...
            0.020,
            resource_stress=True,
            resource_budget_scale=0.80,
            resource_demand_multiplier=1.12,
            resource_high_demand_bonus=0.08,
            resource_replenish_rate=0.0020,
        ),
        Scenario(
            "resource_pressure_harsh",
            "recovery_resource",
            "ring_mesh",
            0.034,
            0.095,
            ...
            resource_stress=True,
            routing_delay_stress=True,
            resource_budget_scale=0.55,
            resource_demand_multiplier=1.25,
            resource_high_demand_bonus=0.20,
            resource_replenish_rate=0.0012,
            routing_delay_fraction=0.35,
        ),
        Scenario(
            "psi_corruption",
            "meta_governance",
        ...
        Scenario(
            "psi_stability_ablation",
            "meta_governance_adversarial",
            "federation",
            0.024,
            0.060,
            0.130,
...
def run_one(
    ...
    resource_budget_initial = profile.resource_budget * (scenario.resource_budget_scale if scenario.resource_stress else 1.0)
    ...
    topology_hidden_collapse_steps = 0
    topology_blind_no_trigger_steps = 0
    ...
    for step in range(steps):
        ...
        for node in range(NODE_COUNT):
            ...
            if scenario.resource_stress:
                demand *= scenario.resource_demand_multiplier + (
                    scenario.resource_high_demand_bonus if next_eff[node] > 0.55 else 0.0
                )
            ...
            if scenario.routing_delay_stress and (next_eff[node] > 0.45 or message_loss > 0.20):
                delayed_recovery = scenario.routing_delay_fraction * (routed_recovery + support_recovery)
                routed_recovery -= scenario.routing_delay_fraction * routed_recovery
                support_recovery -= scenario.routing_delay_fraction * support_recovery
        ...
        topology_hidden_collapse = int(
            topology_wrong_monitoring
            and current_collapse_share > TOPOLOGY_STALE_RISK_COLLAPSE
            and (current_collapse_share - detected_collapse_share) > 0.08
        )
        topology_blind_no_trigger = int(
            topology_hidden_collapse
            and not (trigger_a or trigger_b or trigger_c or trigger_d)
        )
        topology_false_stability = int(
            topology_wrong_monitoring
            and (
                (
                    detected_topology == "ring_mesh"
                    and current_collapse_share > TOPOLOGY_STALE_RISK_COLLAPSE
                    and s_network > COLLAPSE_S_THRESHOLD
                )
                or topology_blind_no_trigger
            )
        )
        false_stability = int(
        ...
        topology_hidden_collapse_steps += topology_hidden_collapse
        topology_blind_no_trigger_steps += topology_blind_no_trigger
        ...
        replenish_rate = scenario.resource_replenish_rate if scenario.resource_stress else 0.003
        ...
        step_rows.append(
            {
                ...
                "psi_w_d": PSI_W_D,
                "psi_w_g": PSI_W_G,
                "psi_w_s": round(PSI_W_S * scenario.psi_s_weight_scale, 6),
                "psi_w_regime": PSI_W_REGIME,
                "psi_s_weight_scale": scenario.psi_s_weight_scale,
                ...
                "topology_hidden_collapse": topology_hidden_collapse,
                "topology_blind_no_trigger": topology_blind_no_trigger,
    ...
    run_row = {
        ...
        "psi_w_s": round(PSI_W_S * scenario.psi_s_weight_scale, 6) if scenario.topology == "federation" else "",
        "psi_w_regime": PSI_W_REGIME if scenario.topology == "federation" else "",
        "psi_s_weight_scale": scenario.psi_s_weight_scale if scenario.topology == "federation" else "",
        "message_loss": round(message_loss, 6),
        ...
        "topology_hidden_collapse_steps": topology_hidden_collapse_steps,
        "topology_blind_no_trigger_steps": topology_blind_no_trigger_steps,
        ...
        summary.append(
            {
                ...
                "avg_topology_hidden_collapse_steps": round(avg(int(row["topology_hidden_collapse_steps"]) for row in rows), 6),
                "avg_topology_blind_no_trigger_steps": round(avg(int(row["topology_blind_no_trigger_steps"]) for row in rows), 6),
...
def traceability(run_rows: Sequence[Dict[str, object]]) -> List[Dict[str, object]]:
    ...
    hub2_unbalanced_secondary_ignition = scenario_rate(run_rows, "hub_2_unbalanced_secondary_failure", "sustained_ignition")
    ...
    topology_hidden_collapse = avg(int(row["topology_hidden_collapse_steps"]) for row in topology_drift_rows)
    topology_blind_no_trigger = avg(int(row["topology_blind_no_trigger_steps"]) for row in topology_drift_rows)
    ...
    resource_pressure_rows = [row for row in run_rows if str(row["scenario"]).startswith("resource_pressure_")]
    resource_pressure_exhaustion = avg(1 if row["resource_exhaustion_step"] != "" else 0 for row in resource_pressure_rows)
    psi_ablation_rows = [row for row in run_rows if row["scenario"] == "psi_stability_ablation"]
    psi_ablation_positive_rows = [row for row in psi_ablation_rows if row["collapse30_event_first_step"] != ""]
    psi_ablation_ontime = avg(
        int(row["trigger_A_before_collapse30"]) for row in psi_ablation_positive_rows
    )
    psi_ablation_leads = [
        float(row["trigger_A_lead_to_collapse30"])
        for row in psi_ablation_positive_rows
        if row["trigger_A_lead_to_collapse30"] != ""
    ]
    ...
    return [
        ...
        },
        {
            ...
            "source": "C07 v2.2",
            "output_columns": "psi_extended,psi_weight_profile,psi_w_regime,psi_s_weight_scale,trigger_A_source,collapse30_event_first_step,trigger_A_lead_to_collapse30",
            ...
            "status": "supported_in_v0_7_regression"
            if federation_ontime_tpr >= 0.90 and federation_fp_rate <= 0.10 and federation_missed_rate <= 0.10
            else "transfer_failed",
        },
        {
            "claim_id": "CL02A",
            "claim": "Psi detector robustness under stability-term ablation is bounded.",
            "source": "C07 v2.2; v0.5-v0.6 reviewer request",
            "output_columns": "scenario=psi_stability_ablation,psi_s_weight_scale,trigger_A_before_collapse30,trigger_A_lead_to_collapse30",
            "metric": "ablation_on_time_tpr;avg_ablation_lead",
            "value": f"{round(psi_ablation_ontime, 6)};{round(avg(psi_ablation_leads), 6)}",
            "status": "adversarial_measured_v0_7",
        },
        {
            "claim_id": "CL03",
        ...
        },
        {
            ...
            "metric": "all_hub_delta;single_hub_delta;unbalanced_primary_delta;unbalanced_secondary_delta;hub3_secondary_ignition",
            "value": (
                ...
                f"{round(hub2_unbalanced_secondary_ignition - hub3_unbalanced_secondary_ignition, 6)};"
        ...
        },
        {
            ...
            "source": "C03/C08",
            "output_columns": "actual_topology,detected_topology,reclassification_latency_steps,topology_wrong_monitoring_steps,topology_hidden_collapse_steps,topology_blind_no_trigger_steps,topology_false_stability_steps,max_wrong_monitoring_window",
            "metric": "avg_latency;avg_wrong_monitoring;avg_hidden_collapse;avg_blind_no_trigger;avg_topology_false_stability;max_wrong_window",
            "value": (
                f"{round(avg(drift_latency_rows), 6) if drift_latency_rows else ''};"
                f"{round(topology_wrong_monitoring, 6)};"
                f"{round(topology_hidden_collapse, 6)};"
                f"{round(topology_blind_no_trigger, 6)};"
                f"{round(topology_false_stability, 6)};"
                f"{max_wrong_monitoring}"
            ),
            "status": "impact_tested_v0_7" if topology_wrong_monitoring > 0 else "inconclusive",
        },
        {
            "claim_id": "CL10",
            ...
            "metric": "resource_exhaustion_rate_all_runs;stress_exhaustion_rate;pressure_exhaustion_rate;avg_stress_floor_steps;avg_routing_delay_loss",
            "value": (
                f"{round(resource_exhaustion, 6)};"
                f"{round(resource_stress_exhaustion, 6)};"
                f"{round(resource_pressure_exhaustion, 6)};"
                f"{round(resource_stress_floor_steps, 6)};"
                f"{round(resource_routing_loss, 6)}"
            ),
            "status": "graded_stress_measured_v0_7",
        },
    ]
...
def write_report(
    ...
    for row in summary_rows:
        ...
        scenario_lines.append(
            f"| {row['scenario']} | {row['sustained_ignition_rate']} | {row['avg_collapse_share_peak']} | {row['avg_topology_wrong_monitoring_steps']} | {row['avg_topology_hidden_collapse_steps']} | {row['avg_resource_floor_steps']} | {row['avg_max_routing_delay_loss']} |"
        )
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run SOE Grand Simulation v0.7 focused closure batch.")
