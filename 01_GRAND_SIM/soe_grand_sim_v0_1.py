...
def detect_topology(actual_topology: str, previous: str, step: int) -> str:
    if step % TOPOLOGY_RECHECK_INTERVAL != 0:
        return previous
...
def apply_scenario_disturbance(
    ...
    if scenario.hub_stress and 28 <= step <= 120:
        ...
        if node < hub_limit or (scenario.topology_drift and node == 0 and step >= 60):
...
def run_one(
    ...
    first_star_adjacent_step: Optional[int] = None
    ...
    for step in range(steps):
        ...
        detected_topology = detect_topology(actual_topology, detected_topology, step)
        if actual_topology in ("star_adjacent", "star") or actual_topology.startswith("hub_2"):
            star_adjacency_persist_steps += 1
            if first_star_adjacent_step is None:
                first_star_adjacent_step = step
        if (
            first_star_adjacent_step is not None
            and reclassification_latency == ""
            and previous_detected != detected_topology
            and detected_topology in ("star_adjacent", "star", "hub_2")
        ):
            reclassification_latency = step - first_star_adjacent_step
        ...
        for node in range(NODE_COUNT):
            ...
            recovery = r_internal + (r_network_raw * resource_scale) + (r_gov * resource_scale) + r_base
            if external_support > 0.0:
                recovery += external_support * max(0.0, 1.0 - trust[node]) * resource_scale
        ...
        regime_weight = 1.0 if current_collapse_share >= IGNITION_SHARE else (0.65 if current_collapse_share >= 0.20 else 0.15)
        observed_t = t_mean
        observed_d = d_mean
        ...
        psi_extended = psi_base + (PSI_LAMBDA * variance if actual_topology == "federation" else 0.0)

        if actual_topology == "federation":
            trigger_a = int(psi_extended >= profile.psi_threshold)
        ...
        false_stability = int(
            ...
            or (actual_topology == "federation" and s_network > 0.52 and max_cluster_collapse_share(state) > 0.35)
        )
        ...
        resource_budget = clamp(resource_budget + 0.003 * g_effective, 0.0, profile.resource_budget)
...
def traceability(run_rows: Sequence[Dict[str, object]]) -> List[Dict[str, object]]:
    ...
    async_false_stability = scenario_rate(run_rows, "async_message_loss", "false_stability_steps")
    drift_latency_rows = [
        float(row["reclassification_latency_steps"])
        for row in run_rows
        if row["scenario"] == "topology_drift" and row["reclassification_latency_steps"] != ""
    ]
    ...
    return [
        ...
        },
        {
            ...
            "source": "C07 v2.2",
            "output_columns": "psi_extended,cluster_variance,trigger_A_source",
            "metric": "federation_psi_detector_run_rate",
            "value": round(fed_detector_rate, 6),
            "status": "supported" if fed_detector_rate >= 0.50 else "inconclusive",
        },
        {
            "claim_id": "CL03",
        ...
        },
        {
            ...
            "source": "C00/C04; v18",
            "output_columns": "hub_2_balanced vs hub_3_balanced sustained_ignition",
            "metric": "hub2_minus_hub3_ignition_rate",
            "value": round(hub2_ignition - hub3_ignition, 6),
            "status": "supported" if hub2_ignition > hub3_ignition else "inconclusive",
        },
        {
            "claim_id": "CL06",
        ...
        },
        {
            ...
            "source": "C08",
            "output_columns": "message_loss,phase_divergence,false_stability_steps",
            "metric": "avg_async_false_stability_steps",
            "value": round(async_false_stability, 6),
            "status": "supported" if async_false_stability > 0 else "inconclusive",
        },
        {
            "claim_id": "CL09",
            "claim": "Topology monitoring must reclassify drift.",
            "source": "C03/C08",
            "output_columns": "actual_topology,detected_topology,reclassification_latency_steps",
            "metric": "avg_reclassification_latency_steps",
            "value": round(avg(drift_latency_rows), 6) if drift_latency_rows else "",
            "status": "measured" if drift_latency_rows else "inconclusive",
        },
        {
            "claim_id": "CL10",
            "claim": "Recovery claims must survive finite resources.",
            "source": "C02/C06/C08",
            "output_columns": "resource_budget,resource_exhaustion_step,max_unmet_recovery_demand",
            "metric": "resource_exhaustion_rate_all_runs",
            "value": round(resource_exhaustion, 6),
            "status": "measured",
        },
    ]
...
def write_report(
    ...
    for row in summary_rows:
        ...
        scenario_lines.append(
            f"| {row['scenario']} | {row['sustained_ignition_rate']} | {row['avg_collapse_share_peak']} | {row['avg_trigger_A_count']} | {row['avg_false_stability_steps']} |"
        )
    ...
    lines = [
        "# SOE Grand Simulation v0.1 Report",
        "",
        "## Batch",
        "",
        f"- Runs: `{len(run_rows)}`.",
        ...
        *trace_lines,
        ...
        "## Artifacts",
        "",
        "- `grand_sim_v0_1_runs.csv`",
        "- `grand_sim_v0_1_steps.csv`",
        "- `grand_sim_v0_1_scenario_summary.csv`",
        "- `grand_sim_v0_1_traceability.csv`",
        "",
    ]
    (output_dir / "grand_sim_v0_1_report.md").write_text("\n".join(lines), encoding="utf-8")
...
def run_batch(runs_per_cell: int, steps: int, seed_start: int, output_dir: pathlib.Path) -> Tuple[List[Dict[str, object]], List[Dict[str, object]], List[Dict[str, object]], List[Dict[str, object]]]:
    ...
    write_csv(output_dir / "grand_sim_v0_1_runs.csv", run_rows)
    write_csv(output_dir / "grand_sim_v0_1_steps.csv", step_rows)
    write_csv(output_dir / "grand_sim_v0_1_scenario_summary.csv", summary_rows)
    write_csv(output_dir / "grand_sim_v0_1_traceability.csv", trace_rows)
    write_report(output_dir, run_rows, summary_rows, trace_rows)
    return run_rows, step_rows, summary_rows, trace_rows


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run SOE Grand Simulation v0.1 first integrated batch.")
    ...
    parser.add_argument("--output-dir", default="grand_sim_v0_1")
...
def main() -> None:
    ...
    if args.smoke:
        runs_per_cell = 1
        steps = 20
        output_dir = pathlib.Path("grand_sim_v0_1_smoke")
